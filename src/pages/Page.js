...
class Page extends React.Component {
  render() {
    const { path, post, posts, nextPost, prevPost } = this.props;
    if ( process.env.NODE_ENV === 'development' ) {
      console.log( post );
    }
    return (
      <ContainerFluid posts={ posts } show={ true }>
        <div className={ styles.page } id={ 'pageTop' }>
          <Helmet>
            <meta charSet="utf-8"/>
            <title>{ post.title } | { post.subtitle } | ComposersCAPE</title>
            <meta name="author" content={ `${ post.createdBy.firstname } ${ post.createdBy.lastname }` }/>
          ...
          <div className={ styles.heading }>
            { post.featuredImage ?
              (
                <Image
                  src={ post.featuredImage.formats.large ? post.featuredImage.formats.large.url : post.featuredImage.url }
            ...
            <h2 className={ styles.date }>
              <span>{ post.createdBy.firstname } { post.createdBy.lastname }</span> { moment( post.created_at ).format( 'MMMM Do, YYYY' ) }
            </h2>
          </div>
